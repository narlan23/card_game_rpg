...
def draw_player_status(surface, player, x, y):
    ...
    # Texto centralizado da vida (com shield se existir)
    if player.shield > 0:
    ...
    if hasattr(player, "status_effects") and player.status_effects:
        icon_size = 16
        spacing = 5
        offset_x = x + bar_width + 10  # ao lado da barra de vida
        offset_y = y + 28

        for i, (effect, data) in enumerate(player.status_effects.items()):
            # Cor do ícone baseada no tipo de efeito
            color = (0, 200, 255) if "buff" in effect.lower() else (200, 50, 50)

            # Desenha quadradinho do efeito
            ...
            # Mostra duração no centro do ícone
